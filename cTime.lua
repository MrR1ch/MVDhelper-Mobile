...
function main()
    ...
    if hour >= 5 and hour <= 10 then chatmsg(string.format("Äîáðîå óòðî, %s! {C3C3C3}(/ctime)", name)) end
    if hour >= 11 and hour <= 16 then chatmsg(string.format("Äîáðûé äåíü, %s! {C3C3C3}(/ctime)", name)) end
    if hour >= 17 and hour <= 22 then chatmsg(string.format("Äîáðûé âå÷åð, %s! {C3C3C3}(/ctime)", name)) end
    if hour >= 23 or hour <= 4 then chatmsg(string.format("Äîáðîé íî÷è, %s! {C3C3C3}(/ctime)", name)) end
    ----------------------------------------------------------------
    while true do
        wait(0)
